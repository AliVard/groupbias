...
def load_dataset(dataset_name, datasets_info, session_cnt):
    ...
    if dataset_info['type'] == 'libsvm':
        ...
        clicks, sessions = lv2clicks(dataset.trlv, dataset.trdlr)
    ...
    outlierness = dataset_info.get('outlierness', None)
    if isinstance(outlierness, int):
        outlierness = dataset.trfm[:, dataset_info['outlierness']]
    elif outlierness is None:
        outlierness = np.zeros_like(dataset.trfm[:,0])
    else:
        with open(outlierness, 'rb') as f:
